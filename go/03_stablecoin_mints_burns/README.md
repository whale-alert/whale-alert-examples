# Go Quickstart: Stablecoin Liquidity & Supply Events (Mints, Burns, Freezes & Locks)
...
2. **Real-Time Cursor Polling**: Spawns concurrent worker goroutines polling each blockchain's transactions endpoint (`GET /{blockchain}/transactions?start_height={height}`) using the `next` pagination cursor.
...
### 2. Run
```bash
cd go/03_stablecoin_mints_burns
go run main.go
```
...
[STABLECOIN LOCK]
   ...
   Blockchain:  Ethereum
   From:        0x1a2b...3c4d
   Locked At:   MakerDAO DSR Contract
   Hash:        0x7b1c...99a2
...
Customize the USD threshold and supported stablecoins directly in `main.go`:
